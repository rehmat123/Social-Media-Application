...
class TimeLines extends Component {
  ...
  componentDidMount() {
    API.get(Globals.api + '/post/public-posts?page=1')
      .then(data => {
  ...
  fetchMoreData(){
    if(this.state.currentPage<= this.state.pageCount  ){
          API.get(Globals.api + '/post/public-posts?page='+this.state.currentPage)
            .then(data => {
              setTimeout(() => {
                this.setState({
                ...
                <div className="box_inner d-flex align-items-start">
                  ...
                  <div className="box_inner_content">
                    <div className="name_div">
                      <h4 className="d-inline-block">{post.full_name} <span>{post.activity} at</span></h4> <span>{post.location}</span>
